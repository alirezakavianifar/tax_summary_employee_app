...
const getRoleBadge = (role: string, roleList?: RoleDto[]) => {
    switch (role) {
        case 'Admin':
            return {
...
export default function UsersPage() {
    ...
    const loadRoles = async () => {
        try {
            const data = await rolesApi.getActiveRoles();
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                        ...
                        <div className="flex items-center gap-3">
                            <Link
                                href="/admin/roles"
                                className="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2.5 rounded-lg transition shadow-sm font-medium text-sm"
                            >
                                        ...
                                        users.map((user) => {
                                            ...
                                            return (
                                                <tr key={user.id} className="hover:bg-gray-50/70 transition">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        {(() => {
                                                            const badge = getRoleBadge(user.role, availableRoles);
                                                            return (
                                                    ...
                                                    <td className="px-6 py-4">
                                                        {user.role === 'Admin' || user.role === 'DirectorGeneral' ? (
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-xs p-4">
                        <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 border border-gray-100 animate-in fade-in zoom-in-95 duration-150">
                            ...
                            <div className="mb-4 bg-gray-50 p-3 rounded-lg text-xs text-gray-600 space-y-1">
                                ...
                                <div><span className="font-semibold text-gray-700">نقش کاربری:</span> {getRoleBadge(resetModalUser.role, availableRoles).label}</div>
                            </div>
                                ...
                                <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[11px] font-semibold text-primary-700 bg-primary-100 px-2 py-0.5 rounded-full">
                                                {editRole === 'Admin' || editRole === 'DirectorGeneral' || editRole === 'Treasury' ? 'دسترسی سراسری' : `${editSelectedOfficeIds.length} اداره انتخاب شده`}
                                            </span>
                                        </div>
                                        {editRole !== 'Admin' && editRole !== 'DirectorGeneral' && editRole !== 'Treasury' && (
                                    ...
                                    {editRole === 'Admin' || editRole === 'DirectorGeneral' || editRole === 'Treasury' ? (
                                        <p className="text-xs text-amber-700 bg-amber-50 p-2.5 rounded-lg border border-amber-200">
                                            کاربران با نقش «{editRole === 'DirectorGeneral' ? 'مدیر کل امور مالیاتی' : editRole === 'Treasury' ? 'ذیحساب' : 'مدیر ارشد سامانه'}» به صورت خودکار به تمامی کاربرگ‌ها و ادارات استان دسترسی کامل دارند.
                                        </p>
                                    ) : (
                                        <>
                                            ...
                                            {loadingOffices ? (
                                                ...
                                                <div className="text-center py-4 space-y-2">
                                                    ...
                                                    {editOfficeSearch.trim() && (
                                                        <button
                                                            type="button"
                                                            onClick={async () => {
                                                                const code = editOfficeSearch.trim();
                                                                ...
                                                                try {
                                                                    ...
                                                                    setEditSelectedOfficeIds(prev => [...prev, newOff.id]);
                                                                    setEditOfficeSearch('');
                                                                } catch (e: any) {
                                                                    setActionError(e.response?.data?.error || 'خطا در ثبت کد جدید');
                                                                }
                                                            }}
                                                        ...
                                                        >
                                                            <span>+</span>
                                                            <span>ایجاد و افزودن کد «{editOfficeSearch.trim()}» به عنوان اداره/گروه مجاز</span>
                                                        </button>
                                                    )}
                                                </div>
                                            ) : (
